...
engine = create_engine("postgresql://postgres:Haruna0419@localhost:5432/postgres")
...
# 4. Gross Margin 表格
def get_all_gross_margin_columns():
    stock_code = pd.read_csv("stock_code.csv")
    code_list = stock_code["0"].astype(str).tolist()
    code_list = [format_code(code) for code in code_list]
    code_list = [code + "_gross_margin" for code in code_list]  # 毛利率
...
def get_all_operating_margin_columns():
    stock_code = pd.read_csv("stock_code.csv")
    code_list = stock_code["0"].astype(str).tolist()
    code_list = [format_code(code) for code in code_list]
    code_list = [code + "_operating_margin" for code in code_list]  # 營業利益率

    columns = ["year", "quarter"] + code_list
    return columns

# 7. P/E Ratio 表格
def get_all_pe_ratio_columns():
    stock_code = pd.read_csv("stock_code.csv")
    code_list = stock_code["0"].astype(str).tolist()
    code_list = [format_code(code) for code in code_list]
    code_list = [code + "_pe_ratio" for code in code_list]  # 本益比

    columns = ["year", "quarter"] + code_list
    return columns

# 8. Free Cash Flow 表格
def get_all_free_cash_flow_columns():
    stock_code = pd.read_csv("stock_code.csv")
    code_list = stock_code["0"].astype(str).tolist()
    code_list = [format_code(code) for code in code_list]
    code_list = [code + "_free_cash_flow" for code in code_list]  # 自由現金流
...
def create_all_composite_tables():
    ...
    # create_table("gross_margin", get_all_gross_margin_columns())
    # 依此類推
